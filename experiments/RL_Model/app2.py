...
class MahimahiTraceManager:
    ...
    """
    def __init__(self, folder_path="traces_folder/mahimahi",split="all", test_size=8, seed=42):
        self.traces = []
        ...
        if len(self.traces) > test_size:
            rng = random.Random(seed)
            shuffled_indices = list(range(len(self.traces)))
            ...
            if split == "train":
                self.traces = [self.traces[i] for i in train_indices]
            elif split == "test":
                self.traces = [self.traces[i] for i in test_indices]
        ...
        print(f"✅ Split '{split}': {len(self.traces)} trace dimuat. Target scaling: {SCALE_TARGET:.2f} Mbps")
...
def evaluate_and_plot(model, eval_env, tm, split_name, log_dir):
    print(f"\n📊 Memulai Evaluasi Per-Trace pada SET {split_name.upper()} ({len(tm.traces)} traces)...")
    
    res_names = ["240p", "360p", "480p", "720p"]
    res_colors = ["#3498db", "#2ecc71", "#f1c40f", "#e74c3c"] 
    
    # Looping evaluasi secara terisolasi untuk setiap trace
    for i in range(len(tm.traces)):
        trace_name = tm.traces[i]["name"]
        history = []
        
        # Mengunci environment untuk menggunakan trace spesifik pada indeks 'i'
        obs, _ = eval_env.reset(options={"trace_idx": i})
        done = False
        ...
        while not done:
            ...
            # Normalisasi output info jika berformat list
            if isinstance(info, list):
                info = info[0]
                
            rb = info.get("reward_breakdown", {})
            
            # Perekaman metrik pada setiap step/segmen untuk trace yang sedang aktif
            history.append({
                "TP":           info.get("raw_tp", 0.0),
                "Volatility":   info.get("volatility", 0.0),
                "Buffer":       info.get("buffer", 0.0),
                "Bitrate":      info.get("bitrate", 0.0),
                "Stalling":     info.get("stalling", 0.0),
                "Vetoed":       info.get("is_vetoed", False),
                "R_Quality":    rb.get("quality", 0.0),
                "R_Buffer":     rb.get("buffer", 0.0),
                "R_Stall":      rb.get("stall", 0.0),
                "R_Smooth":     rb.get("smooth", 0.0),
                "R_Gate_Break": rb.get("gate_break", 0.0),
        ...
        # Konversi rekam jejak tunggal menjadi DataFrame
        df = pd.DataFrame(history)
        
        # Pembangunan 4 panel analitik
        ...
        for br, label, col in zip(eval_env.bitrates, res_names, res_colors):
        ...
        # Modifikasi dinamis agar visualisasi dengan jelas menunjukkan Train atau Test
        ax1.set_title(f"[{split_name.upper()}] Performance Analysis: {trace_name}", fontsize=16, fontweight='bold', pad=20)
        ...
        # Penamaan file secara dinamis memisahkan metadata set dan trace
        out_path = os.path.join(log_dir, f"eval_v4_{split_name}_{trace_name}.png")
...
def run_experiment():
    log_dir = "../../rl_logs_http_baseline_v2/"
    ...
    print(f"  State Dim: 5 Dimensions (RTT Removed)")
    ...
    model.save("../models/http_baseline_4bitrate_model_v2")
    ...
    eval_env_train = PureHTTPStreamingEnv(tm_train) 
    ...
    eval_env_test = PureHTTPStreamingEnv(tm_test)
