...
class HybridStreamingEnvNDN(gym.Env):
    ...
    def step(self, action):
        ...
        # ------------------------------------------------------------------
        # KOREKSI: Menghapus total penjemputan variabel prev_rtt dari state lama
        ...
        # KOREKSI: Blok simulasi kalkulasi matematika RTT dinamis dihapus total.
        # Fokus dialihkan sepenuhnya pada stabilitas kontrol kapasitas jendela (CWND).
        ...
        # B. NDN VETO & SMART UPGRADE (Murni Berbasis CWND Berderau Rendah)
        else:
            if target_idx > current_idx:
                # KOREKSI: Logika rtt_increase_ratio dibuang. Sinyal kemacetan tunggal murni dari CWND.
        ...
        # ------------------------------------------------------------------
        # 6. REWARD FUNCTION (SMOOTH & CAUSAL)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # KOREKSI: Menghapus elemen new_rtt dari penyimpanan state akhir lingkungan
        self.state = np.array(
...
def run_experiment():
    log_dir = "../logs/rl_logs_12bitrate/"
    ...
    model.save("../models/hybrid_4bitrate_ndn_model_v12")
    print("✅ Pelatihan selesai. Model disimpan: hybrid_4bitrate_ndn_model_v12.zip")
...
if __name__ == "__main__":
    ...
    # catatan: Untuk eksperimen ini, terdapat bias terhadap keputusan agent berdasarkan kondisi stalling dan buffer tapi untuk adaptasi kondisi flukatif bandwidth dengan baik tapi tidak adaptif terhadap kondisi stalling yang terjadi secara tiba-tiba. Hal ini menunjukkan bahwa meskipun model dapat belajar untuk mengelola buffer dengan baik, ia mungkin masih kesulitan dalam menghadapi situasi stalling yang tidak terduga, yang merupakan tantangan umum dalam pengembangan model ABR yang robust.