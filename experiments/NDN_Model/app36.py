...
class HybridStreamingEnvNDN(gym.Env):
    ...
    def step(self, action):
        ...
        # ------------------------------------------------------------------
        # 5. EKSEKUSI FISIK SIMULASI DASH
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # MODIFIKASI 6: PROACTIVE REWARD SHAPING & MONOTONIC BUFFER INCENTIVE
        # ------------------------------------------------------------------
        # A. Kualitas Video (QoE)
        reward_quality = np.log2(chosen_bitrate / self.bitrates[0])

        # B. Koreksi Monotonik Insentif Buffer (Semakin tebal semakin dihargai)
        if new_buffer < 15.0:
            reward_buffer = -1.5 * (15.0 - new_buffer) # Penalti linier jika di bawah zona aman
        else:
            # Menggunakan fungsi pertumbuhan monotonik linier: 15s -> +1.0, 30s -> +2.0
            # Ini memaksa agen RL untuk selalu mengunci buffer di kapasitas maksimal (30s)
            reward_buffer = 1.0 + ((new_buffer - 15.0) / 15.0)

        # C. Proactive Predictive Penalty (Meramal pengurasan sebelum tabrakan stalling)
        # Menghitung berapa detik buffer terkuras pada langkah ini
        buffer_drain = download_time - 5.0 
        if buffer_drain > 0 and new_buffer < 25.0:
            # Jika waktu unduh melebihi durasi segmen, berikan penalti proaktif 
            # proporsional terhadap laju pengurasan, meskipun stalling riil masih 0.
            penalty_proactive_drain = -0.4 * buffer_drain
        else:
            penalty_proactive_drain = 0.0

        # D. Reactive Physical Stalling Penalty (Tetap dipertahankan sebagai benteng terakhir)
        penalty_stalling = 0.0
        if stalling > 0:
            penalty_stalling = - (10.0 + 15.0 * np.power(stalling, 1.2))

        # E. Kelancaran & Veto Penalty
        penalty_smoothness = -2.0 * abs(executed_idx - current_idx)
        penalty_veto = -0.3 if is_vetoed else 0.0

        # Akumulasi Fungsi Reward Baru
...
def run_experiment():
    log_dir = "../logs/rl_logs_13bitrate/"
    ...
    model.save("../models/hybrid_4bitrate_ndn_model_v13")
    print("✅ Pelatihan selesai. Model disimpan: hybrid_4bitrate_ndn_model_v13.zip")
...
if __name__ == "__main__":
    ...
    # catatan: hasil nya fungsi 4  aturan veto itu terlalu ketat dan menciptakan cursed enviroment karena jika gagal upgrade sekali saja maka akan sulit untuk naik ke bitrate yang lebih tinggi lagi karena aturan veto yang terlalu agresif. Hal ini menunjukkan bahwa meskipun aturan veto dapat membantu mencegah stalling, jika terlalu ketat, ia dapat menghambat kemampuan model untuk belajar dan beradaptasi dengan kondisi jaringan yang berubah-ubah, sehingga menciptakan lingkungan pelatihan yang sulit untuk dioptimalkan.
