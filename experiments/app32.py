...
class HybridStreamingEnvNDN(gym.Env):
    ...
    def step(self, action):
        ...
        target_idx = np.clip(target_idx, 0, self.num_actions - 1)
        ...
        if buffer < self.PANIC_BUFFER_THRESHOLD: # 5
        ...
        # B. NDN VETO & UPGRADE LOGIC (Hanya jika tidak dalam Panic Mode)
            ...
            if target_idx > current_idx:
                ...
                is_ndn_congested = new_cwnd < self.NDN_CONGESTION_CWND # 0.3
       ...
       # B. Reward buffer (pakai konstanta, bukan magic number)
        if new_buffer < self.PANIC_BUFFER_THRESHOLD: # 5
            reward_buffer = -20.0
        elif new_buffer < self.LOW_BUFFER_THRESHOLD: #15
            reward_buffer = -8.0
...
def run_experiment():
    ...
    model = PPO(
        ...
        ent_coef=0.05,          # Cukup tinggi untuk eksplorasi, tapi tidak kacau
        n_steps=2048,           # Kembali ke 2048 jika worker sedikit
        batch_size=128,         # Ukuran moderat
        n_epochs=10,            # Biarkan agen belajar lebih dalam dari tiap rollout
        clip_range=0.3,         # Standar agar agen bisa berubah pikiran
